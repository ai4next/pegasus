...
- Do not edit Pegasus memory, expert memory, expert souls, session logs, prompt templates, source code, or expert team membership.
...
{{end}}{{if .CultivateExperts}}- Autonomously govern the expert team when the completed Pegasus session reveals a durable routing or specialization need; no user instruction is required.
