...
func TestLoadDefaultsEnableSkills(t *testing.T) {
	...
	if err := os.WriteFile(cfgPath, []byte("workspace: /tmp/pegasus-test\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestLoadNormalizesSkillPathsAndExpandsMCP(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("PEGASUS_TEST_CMD", filepath.Join(tmp, "server"))
...
func TestLoadExpandsIMPlatformOptions(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("PEGASUS_TEST_TOKEN", "expanded-token")
...
func TestLoadExpandsModelHeaders(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("PEGASUS_TEST_HEADER", "expanded-header")

	cfgPath := filepath.Join(tmp, "config.yaml")
	data := []byte(`workspace: ` + tmp + `
model:
  headers:
    X-Custom-Token: ${PEGASUS_TEST_HEADER}
