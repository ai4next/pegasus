...
//   - Pegasus → memory/pegasus, session/pegasus, state/{expert}/soul.md
//   - Expert   → memory/{expert}, session/{expert}, state/{expert}/soul.md
type Evolution struct {
	pegasusRunner *runner.Runner
...
func NewEvolution(llm model.LLM, session adksession.Service) (*Evolution, error) {
	pegasusRunner, err := newEvolutionRunner(llm, pegasusagent.PegasusEvolverName, pegasusagent.NewEvolver)
	if err != nil {
		return nil, err
	}
	expertRunner, err := newEvolutionRunner(llm, pegasusagent.ExpertEvolverName, pegasusagent.NewExpertEvolver)
	if err != nil {
		return nil, err
	}
	metaRunner, err := newEvolutionRunner(llm, pegasusagent.MetaEvolverName, pegasusagent.NewMetaEvolver)
	if err != nil {
		return nil, err
	}

	return &Evolution{
		pegasusRunner: pegasusRunner,
...
func newEvolutionRunner(llm model.LLM, appName string, factory evolverFactory) (*runner.Runner, error) {
	...
	sessionService, err := pegasussession.NewServiceInRoot(global.AgentStateDir(appName))
	if err != nil {
...
func (e *Evolution) runMetaEvolution(ctx context.Context, agentEvolutionSessionID string) error {
	...
	scope := evolutionScopeFromSignal(hook.EvolutionSignal{
		...
		AgentName: pegasusagent.MetaEvolverName,
		Role:      "meta",
		RootDir:   global.AgentStateDir(pegasusagent.MetaEvolverName),
	})
...
func (e *Evolution) runnerFor(scope evolutionScope) (*runner.Runner, error) {
	...
	if e.pegasusRunner == nil {
		return nil, fmt.Errorf("missing pegasus evolution runner")
	}
	return e.pegasusRunner, nil
}
