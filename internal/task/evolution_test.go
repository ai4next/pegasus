...
func TestEvolutionLoopPublishesFailedEvent(t *testing.T) {
	...
	e.SetBroker(broker)
	go e.Loop(ctx)
	e.signal <- hook.EvolutionSignal{SessionID: "s1", Role: "pegasus"}
...
func TestNewEvolutionCreatesPersistentEvolutionRoot(t *testing.T) {
	...
	if e.pegasusRunner == nil || e.expertRunner == nil {
	...
	for _, path := range []string{
		...
		filepath.Join(global.AgentMemoryDir("pegasus-evolver"), "l1.toml"),
		filepath.Join(global.AgentMemoryDir("pegasus-evolver"), "l2"),
		global.AgentSessionsDir("pegasus-evolver"),
		global.AgentStateDBPath("pegasus-evolver"),
...
func TestEvolutionDataFromMetaSignalUsesEvolutionScope(t *testing.T) {
	...
	data := evolutionDataFromSignal(hook.EvolutionSignal{
		SessionID: "agent-evolution-pegasus-pegasus-1",
	...
	if data.SessionLogPath != filepath.Join(global.AgentSessionsDir("meta-evolver"), "agent-evolution-pegasus-pegasus-1.log") {
...
func TestEvolutionDataFromPegasusSignalUsesWorkspaceScope(t *testing.T) {
	workspace := t.TempDir()
	global.SetConfig(&config.Config{
		Workspace: workspace,
	...
	if data.SessionLogPath != filepath.Join(global.AgentSessionsDir("pegasus"), "s1.log") {
		t.Fatalf("SessionLogPath = %q", data.SessionLogPath)
	}
	if data.L1Path != filepath.Join(global.AgentMemoryDir("pegasus"), "l1.toml") {
		t.Fatalf("L1Path = %q", data.L1Path)
	}
	if data.SOPDir != filepath.Join(global.AgentMemoryDir("pegasus"), "l2") {
		t.Fatalf("SOPDir = %q", data.SOPDir)
	}
	...
	if data.SoulPath != "" || data.CanEditSoul {
		t.Fatalf("pegasus scope should not edit a soul directly: soul=%q edit=%v", data.SoulPath, data.CanEditSoul)
	}
	prompt, err := renderEvolutionPrompt(data)
	if err != nil {
		t.Fatal(err)
	...
	if !strings.Contains(prompt, "Autonomously govern the expert team") {
		t.Fatalf("pegasus prompt should authorize autonomous expert governance: %q", prompt)
	}
	if strings.Contains(prompt, "explicitly allows") {
		t.Fatalf("pegasus prompt should not require explicit user instruction: %q", prompt)
	}
}
...
func TestEvolutionDataFromExpertSignalUsesExpertScope(t *testing.T) {
	...
	if strings.Contains(prompt, "Autonomously govern the expert team") {
		t.Fatalf("expert prompt should not include pegasus governance instruction: %q", prompt)
	}
}
