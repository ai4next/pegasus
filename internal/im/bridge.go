...
func (b *AgentBridge) runAndReply(ctx context.Context, client *Client, msg *Message, sessionID, prompt string) error {
	...
	req := pegasusruntime.RunRequest{
		AppName:    b.Config.Session.AppName,
		UserID:     UserID(msg),
		SessionID:  sessionID,
		Message:    genai.NewContentFromText(prompt, genai.RoleUser),
		StateDelta: pegasusruntime.PromptStateDelta(b.Config.Workspace, prompt),
		...
		Compact: pegasusruntime.SessionCompactor(b.SessionService, b.Config.Session.MaxTurns),
	}

	for event, err := range pegasusruntime.StreamRun(ctx, b.Runner, req, nil) {
		if err != nil {
			return err
		}
		switch event.Type {
