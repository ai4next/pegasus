...
// BuildConfig describes one concrete agent instance. Pegasus and experts use
...
// explain Pegasus's external abilities without opening network/process-backed
// toolsets.
...
// DescribeConfiguredToolsets returns the ADK toolsets Pegasus will attach for
...
// NewFromConfig creates an agent with the shared Pegasus runtime wiring:
...
func New(llm model.LLM, cfg *config.Config, memSvc *memory.Service, sessionSvc adksession.Service, expertRegistry *expert.Registry, delegateRunner tool.DelegateRunner, evolutionCh chan<- hook.EvolutionSignal) (adkagent.Agent, []*plugin.Plugin, error) {
	return NewFromConfig(llm, cfg, BuildConfig{
		Name:              "pegasus",
		Instruction:       prompt.PegasusSystem(),
