...
func TestSessionContextContentsIncludesHistoricalContext(t *testing.T) {
	contents := sessionContextContents(sessionContext{
		...
		Messages: []pegasussession.Message{
			{Role: pegasussession.MessageUser, Content: "old user request"},
			{Role: pegasussession.MessageAssistant, Content: "old assistant answer"},
			{Role: pegasussession.MessageUser, Content: "current request"},
		},
	})
...
func TestSessionContextContentsSnipsMiddleMessages(t *testing.T) {
	var messages []pegasussession.Message
	for i := range 8 {
		messages = append(messages, pegasussession.Message{
...
func TestSessionContextContentsMicroCompactsOldToolResults(t *testing.T) {
	...
	for i := range 5 {
		window.Messages = append(window.Messages, pegasussession.Message{
			Role:     pegasussession.MessageTool,
			ToolName: "read",
			Result:   long + string(rune('0'+i)),
		})
	}
	window.Messages = append(window.Messages, pegasussession.Message{Role: pegasussession.MessageUser, Content: "current"})
...
func TestCompactSessionContextAutoCompactsOversizedWindow(t *testing.T) {
	window := sessionContext{
		MaxMessages: 12,
		Messages: []pegasussession.Message{
			{Role: pegasussession.MessageUser, Content: strings.Repeat("a", contextAutoCompactLimitRunes+1)},
			{Role: pegasussession.MessageAssistant, Content: "recent answer"},
			{Role: pegasussession.MessageUser, Content: "current request"},
		},
	}
