...
import (
	...
	pegasustool "github.com/ai4next/pegasus/internal/tool"
)
...
var _ pegasustool.DelegateRunner = (*dynamicExpertManager)(nil)
...
func TestNewSequentialAgentOrganizesPlanExecuteLoop(t *testing.T) {
	a, err := newSequentialAgent(builtinFakeLLM{}, BuildConfig{Name: "pegasus", Instruction: "base instruction"})
	if err != nil {
		t.Fatal(err)
	}
	if a.Name() != "pegasus" {
	...
	if subagents[0].Name() != "pegasus_planner" {
		t.Fatalf("planner name = %q", subagents[0].Name())
	}
	if subagents[1].Name() != "pegasus_plan_execute_loop" {
	...
	if loopSubagents[0].Name() != "pegasus_executor" {
		t.Fatalf("executor name = %q", loopSubagents[0].Name())
	}
	if loopSubagents[1].Name() != "pegasus_replanner" {
...
func (testCallbackContext) AgentName() string { return "pegasus" }
