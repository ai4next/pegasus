...
func formatPromptQueue(queue []pegasussession.QueuedPrompt, limit int) string {
	if len(queue) == 0 {
...
func formatSessions(sessions []pegasussession.Metadata, currentID string, limit int) string {
	if len(sessions) == 0 {
...
func formatToolsets(toolsets []pegasusagent.ToolsetDescriptor, limit int) string {
	if len(toolsets) == 0 {
...
func formatMessageSearchResults(query string, results []pegasussession.MessageSearchResult, limit int) string {
...
func formatFileRevisions(revisions []pegasussession.FileRevision, limit int) string {
	if len(revisions) == 0 {
