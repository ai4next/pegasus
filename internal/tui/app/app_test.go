...
func TestFormatToolsetsLimit(t *testing.T) {
	got := formatToolsets([]pegasusagent.ToolsetDescriptor{
...
func TestSearchDialogInsertReference(t *testing.T) {
	...
	m.searchDialog = &searchDialogState{
		...
		Results: []pegasussession.MessageSearchResult{
			{
				Metadata: pegasussession.Metadata{SessionID: "1", Title: "Cache Work"},
				Message:  pegasussession.Message{Role: pegasussession.MessageUser, Content: "Recall cache invalidation policy"},
...
func TestRecordPromptSessionReferences(t *testing.T) {
	...
	if len(refs) != 1 || refs[0].SessionID != "past" || refs[0].Role != pegasussession.MessageUser || refs[0].Preview != "historical cache decision" {
		t.Fatalf("refs = %#v", refs)
	}
}

func newTestQueuedSession(t *testing.T) (*pegasussession.Service, *config.Config) {
	t.Helper()
	cfg := &config.Config{
		Workspace: t.TempDir(),
		Model:     config.ModelConfig{Provider: "test", Name: "model"},
