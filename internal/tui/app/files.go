...
func (m *Model) insertFileReference(path string) {
	...
	if err := pegasussession.RecordFileRead(m.sessionService, m.cfg.Session.AppName, "tui-user", m.sessionID, abs); err == nil {
...
func (m *Model) recordPromptFileReferences(prompt string) int {
	...
	count := 0
	for _, ref := range pegasussession.ExtractFileReferences(prompt) {
		path := pegasussession.ResolveWorkspacePath(m.cfg.Workspace, ref)
		if path == "" {
			continue
		}
		if err := pegasussession.RecordFileRead(m.sessionService, m.cfg.Session.AppName, "tui-user", m.sessionID, path); err == nil {
			count++
		}
	}
	if count > 0 {
...
func (m *Model) recordPromptSessionReferences(prompt string) int {
	...
	count := 0
	for _, ref := range pegasussession.ExtractSessionReferences(prompt) {
		if err := pegasussession.RecordSessionReference(m.sessionService, m.cfg.Session.AppName, "tui-user", m.sessionID, ref); err == nil {
			count++
		}
	}
	return count
