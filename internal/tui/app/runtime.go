...
func (m *Model) compactor() pegasusruntime.Compactor {
	return pegasusruntime.SessionCompactor(m.sessionService, m.cfg.Session.MaxTurns)
}
func startAgent(ctx context.Context, run *runner.Runner, broker bus.Broker, appName, sessionID, prompt string, compactor pegasusruntime.Compactor) tea.Cmd {
	return func() tea.Msg {
		go runAgent(ctx, run, broker, appName, sessionID, prompt, compactor)
		return nil
	}
}
func startConfirmation(ctx context.Context, run *runner.Runner, broker bus.Broker, appName, sessionID, confirmationID string, confirmed bool, compactor pegasusruntime.Compactor) tea.Cmd {
	return func() tea.Msg {
...
func runAgent(ctx context.Context, run *runner.Runner, broker bus.Broker, appName, sessionID, prompt string, compactor pegasusruntime.Compactor) {
	msg := genai.NewContentFromText(prompt, genai.RoleUser)
	for _, evtErr := range pegasusruntime.StreamRun(ctx, run, pegasusruntime.RunRequest{
		AppName:    appName,
		UserID:     "tui-user",
		SessionID:  sessionID,
		Message:    msg,
		StateDelta: pegasusruntime.PromptStateDelta("", prompt),
		Config:     agent.RunConfig{},
		Compact:    compactor,
		LoopDetection: pegasusruntime.LoopDetectionConfig{
			Enabled:    true,
			WindowSize: pegasusruntime.DefaultLoopWindowSize,
			MaxRepeats: pegasusruntime.DefaultLoopMaxRepeats,
		},
	}, broker) {
		if evtErr != nil {
			return
...
func runConfirmation(ctx context.Context, run *runner.Runner, broker bus.Broker, appName, sessionID, confirmationID string, confirmed bool, compactor pegasusruntime.Compactor) {
