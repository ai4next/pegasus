...
func TestSearchFindsPegasusAndExpertMemory(t *testing.T) {
	workspace := t.TempDir()
	setTestConfig(t, workspace)
	writeFile(t, filepath.Join(workspace, "memory", "pegasus", "l1.toml"), "[project]\npolicy = \"cache invalidation uses version keys\"\n")
	...
	if !owners[OwnerPegasus] || !owners["reviewer"] {
		t.Fatalf("owners = %#v, want pegasus and reviewer", owners)
	}
}
