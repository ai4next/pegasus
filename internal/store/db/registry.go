...
func NewDBRegistry(paths RegistryPaths) (*DBRegistry, error) {
	...
	pegasusDB, err := OpenPath(paths.PegasusPath)
	if err != nil {
		return nil, fmt.Errorf("open pegasus db: %w", err)
	}
...
func openExpertDBs(expertsDir string) (map[string]*DB, error) {
	...
	for _, entry := range entries {
		if !entry.IsDir() || entry.Name() == "pegasus" {
			continue
		}
