...
func TestEvolutionEvents(t *testing.T) {
	started := EvolutionStarted("s1", "pegasus")
	if started.Type != EventEvolutionStarted || started.SessionID != "s1" || started.Role != "pegasus" {
		t.Fatalf("started = %+v", started)
	}
	finished := EvolutionFinished("s1", "pegasus", "/tmp/projection.md")
	if finished.Type != EventEvolutionFinished || finished.Path != "/tmp/projection.md" {
		t.Fatalf("finished = %+v", finished)
	}
	failed := EvolutionFailed("s1", "pegasus", assertErr("boom"))
