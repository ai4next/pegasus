...
import (
	...
	"github.com/ai4next/pegasus/internal/agent"
	...
	"github.com/ai4next/pegasus/internal/orchestrator"
	pegasusruntime "github.com/ai4next/pegasus/internal/runtime"
	pegasussession "github.com/ai4next/pegasus/internal/session"
	"github.com/ai4next/pegasus/internal/tool"
)

// delegateService runs experts through the same agent builder as Pegasus.
...
func (ds *delegateService) runDelegateSync(ctx context.Context, specName string, task string) (string, error) {
	...
	sessionService, err := pegasussession.NewServiceInRoot(global.ExpertDir(spec.Name))
	if err != nil {
	...
	for event, evtErr := range pegasusruntime.StreamRun(ctx, r, req, nil) {
...
func buildDelegateRunRequest(cfg *config.Config, sessionService adksession.Service, expertName string, task string) pegasusruntime.RunRequest {
	return pegasusruntime.RunRequest{
		AppName:    cfg.Session.AppName + "-expert",
		UserID:     "expert-user",
		Message:    genai.NewContentFromText(task, genai.RoleUser),
		StateDelta: pegasusruntime.PromptStateDelta(cfg.Workspace, task),
		...
		Compact: pegasusruntime.SessionCompactor(sessionService, cfg.Session.MaxTurns),
	}
}
