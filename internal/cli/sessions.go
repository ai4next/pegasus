...
var sessionsCmd = &cobra.Command{
	Use:   "sessions",
	Short: "Inspect persistent Pegasus sessions",
}
...
func writeSessionList(w io.Writer, svc adksession.Service, cfg *config.Config, userID string, asJSON bool) error {
	sessions := pegasussession.ListSessionMetadata(svc, cfg.Session.AppName, userID)
	if asJSON {
...
func writeSessionShow(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID string, asJSON bool) error {
	messages, err := pegasussession.Messages(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return err
	}
	if asJSON {
...
func writeSessionSearch(w io.Writer, svc adksession.Service, cfg *config.Config, userID, query, sessionID, role string, limit int, asJSON bool) error {
	...
	results, err := pegasussession.SearchMessages(svc, cfg.Session.AppName, userID, pegasussession.MessageSearchOptions{
		Query:     query,
		SessionID: sessionID,
		Roles:     roles,
		Limit:     limit,
...
func writeSessionFiles(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID string, asJSON bool) error {
	files, err := pegasussession.SessionFiles(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return err
	}
	if asJSON {
...
func writeSessionHistory(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID string, asJSON bool) error {
	revisions, err := pegasussession.FileRevisions(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return err
	}
	if asJSON {
...
func writeSessionDiff(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID, path string, asJSON bool) error {
	...
	before, beforeMissing, err := pegasussession.FileSnapshotContent(revision.Before)
	if err != nil {
		return err
	}
	after, afterMissing, err := pegasussession.FileSnapshotContent(revision.After)
	if err != nil {
		return err
	}
	diff := pegasussession.UnifiedDiff(revision.Path, before, after)
	if asJSON {
...
func latestSessionFileRevision(svc adksession.Service, cfg *config.Config, userID, sessionID, path string) (pegasussession.FileRevision, bool, error) {
	target, err := filepath.Abs(path)
	if err != nil {
		return pegasussession.FileRevision{}, false, fmt.Errorf("invalid path: %w", err)
	}
	revisions, err := pegasussession.FileRevisions(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return pegasussession.FileRevision{}, false, err
	}
	...
	return pegasussession.FileRevision{}, false, nil
}
...
func writeSessionRevert(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID, path string, asJSON bool) error {
	...
	var revertRevision pegasussession.FileRevision
	if beforeMissing {
		if err := os.Remove(revision.Path); err != nil && !os.IsNotExist(err) {
			return err
		}
		revertRevision, err = pegasussession.RecordFileRevisionWithMissing(svc, cfg.Session.AppName, userID, sessionID, revision.Path, "revert", current, "", currentMissing, true)
	} else {
		...
		revertRevision, err = pegasussession.RecordFileRevisionWithMissing(svc, cfg.Session.AppName, userID, sessionID, revision.Path, "revert", current, before, currentMissing, false)
	}
	if err != nil {
		return err
	}
...
type sessionExport struct {
	Metadata      pegasussession.Metadata            `json:"metadata"`
	Messages      []pegasussession.Message           `json:"messages"`
	Files         []pegasussession.SessionFile       `json:"files,omitempty"`
	FileRevisions []pegasussession.FileRevision      `json:"file_revisions,omitempty"`
	FileChanges   []pegasussession.FileChangeSummary `json:"file_changes,omitempty"`
	PromptQueue   []pegasussession.QueuedPrompt      `json:"prompt_queue,omitempty"`
	References    []pegasussession.SessionReference  `json:"references,omitempty"`
}
...
func buildSessionExport(svc adksession.Service, cfg *config.Config, userID, sessionID string) (sessionExport, error) {
	meta, err := pegasussession.SessionMetadata(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
	...
	changes, err := pegasussession.SessionFileChanges(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return sessionExport{}, err
	}
	queue, err := pegasussession.PromptQueue(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return sessionExport{}, err
	}
	references, err := pegasussession.SessionReferences(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return sessionExport{}, err
	}
...
func writeSessionExportMarkdown(w io.Writer, export sessionExport) error {
	fmt.Fprintf(w, "# Pegasus Session Export\n\n")
...
func writeSessionImport(w io.Writer, svc adksession.Service, cfg *config.Config, userID, path string, overwrite bool, asJSON bool) error {
	...
	meta, err := pegasussession.Import(svc, cfg.Session.AppName, userID, pegasussession.ImportData{
...
func readSessionImportJSONL(r io.Reader) (sessionExport, error) {
	...
	for scanner.Scan() {
		...
		case "message":
			var msg pegasussession.Message
		...
		case "file":
			var file pegasussession.SessionFile
		...
		case "file_revision":
			var revision pegasussession.FileRevision
		...
		case "queued_prompt":
			var queued pegasussession.QueuedPrompt
		...
		case "session_reference":
			var ref pegasussession.SessionReference
...
func compactOptions(cfg *config.Config) pegasussession.CompactOptions {
	opts := pegasussession.CompactOptions{
...
func writeSessionCompact(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID string, opts pegasussession.CompactOptions, asJSON bool) error {
	result, err := pegasussession.Compact(svc, cfg.Session.AppName, userID, sessionID, opts)
	if err != nil {
		return err
	}
...
func writeSessionRename(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID, title string, asJSON bool) error {
	if err := pegasussession.Rename(svc, cfg.Session.AppName, userID, sessionID, title); err != nil {
		return err
	}
	meta, err := pegasussession.SessionMetadata(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
...
func writeSessionQueueAdd(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID, prompt string, asJSON bool) error {
	queued, err := pegasussession.EnqueuePrompt(svc, cfg.Session.AppName, userID, sessionID, prompt)
	if err != nil {
		return err
	}
	if asJSON {
...
func writeSessionQueueClear(w io.Writer, svc adksession.Service, cfg *config.Config, userID, sessionID string, asJSON bool) error {
	cleared, err := pegasussession.ClearPromptQueue(svc, cfg.Session.AppName, userID, sessionID)
	if err != nil {
		return err
	}
	if asJSON {
...
func writeSessionStorage(w io.Writer, svc adksession.Service, _ *config.Config, asJSON bool) error {
	stats, err := pegasussession.StorageStatsFor(svc)
	if err != nil {
		return err
	}
	if asJSON {
...
func writeSessionStorageGC(w io.Writer, svc adksession.Service, _ *config.Config, dryRun bool, asJSON bool) error {
	result, err := pegasussession.CleanupOrphanSnapshots(svc, dryRun)
	if err != nil {
		return err
	}
	if asJSON {
...
func parseMessageRoles(value string) ([]pegasussession.MessageRole, error) {
	...
	roles := make([]pegasussession.MessageRole, 0, len(parts))
	for _, part := range parts {
		switch role := pegasussession.MessageRole(strings.ToLower(strings.TrimSpace(part))); role {
		case pegasussession.MessageUser, pegasussession.MessageAssistant, pegasussession.MessageTool, pegasussession.MessageError:
