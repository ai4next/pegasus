...
func TestWriteSessionExportFormats(t *testing.T) {
	...
	if err := svc.RecordSessionReference("app", "cli-user", "1", pegasussession.SessionReference{
		SessionID: "past",
	...
	if out := buf.String(); !strings.Contains(out, "# Pegasus Session Export") || !strings.Contains(out, "hello from cli") || !strings.Contains(out, "File Changes") || !strings.Contains(out, "Session References") || !strings.Contains(out, "past") {
...
func TestWriteSessionCompact(t *testing.T) {
	...
	var buf bytes.Buffer
	if err := writeSessionCompact(&buf, svc, cfg, "cli-user", "1", pegasussession.CompactOptions{
		MaxMessages: 3,
		KeepLast:    2,
	}, false); err != nil {
		t.Fatal(err)
...
func TestWriteSessionImportJSON(t *testing.T) {
	...
	if err := src.RecordSessionReference("app", "cli-user", "1", pegasussession.SessionReference{
		SessionID: "past",
		Role:      pegasussession.MessageUser,
		Preview:   "historical cache decision",
	}); err != nil {
...
func TestWriteSessionStorageAndGC(t *testing.T) {
	...
	orphanPath := filepath.Join(global.AgentSessionsDir("pegasus"), "snapshots", "aa", "aa-orphan")
...
func newCLITestSession(t *testing.T) (*pegasussession.Service, *config.Config) {
	t.Helper()
...
func newEmptyCLITestSessionService(t *testing.T) (*pegasussession.Service, *config.Config) {
	t.Helper()
	workspace := t.TempDir()
	cfg := &config.Config{
		Workspace: workspace,
