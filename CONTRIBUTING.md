...
Thank you for contributing to Pegasus. Pegasus is a Go-based autonomous AI agent CLI with a terminal UI, persistent sessions, tools, expert delegation, runtime audit logs, instant-messaging integrations, and layered self-evolution. Contributions should keep boundaries clear, behavior testable, and configuration compatible.
...
```bash
git clone https://github.com/ai4next/pegasus.git
cd pegasus
go mod download
go test ./...
```
...
Releases are handled by GitHub Actions when a `v*` tag is pushed. The workflow builds `pg` binaries for Linux, macOS, and Windows. Most contributors do not need to publish releases manually.
